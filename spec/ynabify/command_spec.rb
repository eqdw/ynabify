...
describe Ynabify::Command do
  let( :argv    ) { [ "command", "-a", "flag", "-A", "nother_flag"] }

  context "self" do
    context ".execute" do
      after(:each) do 
        Ynabify::Command.execute(argv)
      end

      it "should initialize a new command" do
        expect(Ynabify::Command).to receive(:new).and_return(spy())
      end

      it "should call execute on it" do
        expect_any_instance_of(Ynabify::Command).to receive(:execute).and_return(nil)
      end
    end
  end
  ...
  context "#initialize" do 
    it "should parse the arguments" do
      expect_any_instance_of(Ynabify::Command).to receive(:parse_flags).with(argv)
      Ynabify::Command.new(argv)
    end
  end

  context "#parse_flags" do
    let( :command      ) { Ynabify::Command.new(argv) }
